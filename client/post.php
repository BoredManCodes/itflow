...
if (isset($_POST['add_ticket'])) {
    ...
    appNotify("Ticket", "New ticket from " . escapeSql($session_client_name) . ": $subject", "/agent/ticket.php?ticket_id=$ticket_id&client_id=$session_client_id", $session_client_id, $ticket_id);
...
if (isset($_GET['close_ticket'])) {
    ...
    if (verifyContactTicketAccess($ticket_id, "Open")) {
        ...
        // Fully close ticket
        mysqli_query($mysqli, "UPDATE tickets SET ticket_status = 5, ticket_resolved_at = COALESCE(ticket_resolved_at, NOW()), ticket_closed_at = NOW() WHERE ticket_id = $ticket_id AND ticket_client_id = $session_client_id");
        syncTicketSlaClock($ticket_id);
