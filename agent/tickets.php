...
if ($status_filter) {
    ...
    // A closed ticket should always be resolved too, but don't rely on that -
    // ticket_status = 5 belongs in Closed even if ticket_resolved_at got missed
    $ticket_status_snippet = 'ticket_resolved_at IS NOT NULL OR ticket_status = 5';
} elseif ($state == 'all') {
    $ticket_status_snippet = '1 = 1';
} else {
    $ticket_status_snippet = 'ticket_resolved_at IS NULL AND ticket_status != 5';
}
...
$total_tickets_open = intval(mysqli_fetch_row(mysqli_query($mysqli, "SELECT COUNT(ticket_id) FROM tickets WHERE ticket_resolved_at IS NULL AND ticket_status != 5 $count_where"))[0]);
$total_tickets_closed = intval(mysqli_fetch_row(mysqli_query($mysqli, "SELECT COUNT(ticket_id) FROM tickets WHERE (ticket_resolved_at IS NOT NULL OR ticket_status = 5) $count_where"))[0]);
$total_tickets_unassigned = intval(mysqli_fetch_row(mysqli_query($mysqli, "SELECT COUNT(ticket_id) FROM tickets WHERE ticket_assigned_to = 0 AND ticket_resolved_at IS NULL AND ticket_status != 5 $count_where"))[0]);
$user_active_assigned_tickets = intval(mysqli_fetch_row(mysqli_query($mysqli, "SELECT COUNT(ticket_id) FROM tickets WHERE ticket_assigned_to = $session_user_id AND ticket_resolved_at IS NULL AND ticket_status != 5 $count_where"))[0]);
