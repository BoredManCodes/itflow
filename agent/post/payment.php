...
if (isset($_POST['add_payment_stripe'])) {
    ...
    if ($payment_succeeded) {
        ...
        logAudit("Invoice", "Payment", "$session_name initiated $provider_name payment amount of " . numfmt_format_currency($currency_format, $invoice_amount, $invoice_currency_code) . " added to invoice $invoice_prefix$invoice_number - $pi_id $extended_log_desc", $client_id, $invoice_id);
        triggerCustomAction('invoice_pay', $invoice_id);
...
if (isset($_GET['refund_payment_square'])) {
    ...
    $payment_id = intval($_GET['refund_payment_square']);

    // payments has no client column - the client comes from the invoice the payment sits on
    $sql = mysqli_query($mysqli,"SELECT invoice_client_id, invoice_number, invoice_prefix, payment_invoice_id, payment_amount, payment_currency_code, payment_reference FROM payments
        LEFT JOIN invoices ON payment_invoice_id = invoice_id
        WHERE payment_id = $payment_id
    ...
    $square_payment_id = str_replace('Square - ', '', $row['payment_reference']);

    $square_provider = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT payment_provider_private_key, payment_provider_public_key FROM payment_providers WHERE payment_provider_name = 'Square' LIMIT 1"));
    if (!$square_provider) {
        exit("Square not enabled / configured");
    }
    $square_secret_key = $square_provider['payment_provider_private_key'];
    $square_public_key = $square_provider['payment_provider_public_key'];

    // Refund
    try {
        require_once __DIR__ . '/../../includes/square_api.php';

        squareApiRequest('POST', '/v2/refunds', $square_secret_key, squareIsSandbox($square_public_key), [
            'idempotency_key' => bin2hex(random_bytes(16)),
            'payment_id' => $square_payment_id,
            'amount_money' => [
                'amount' => (int) round($payment_amount * 100),
                'currency' => strtoupper($payment_currency_code),
            ],
        ]);
    } catch (Exception $e) {
        $error = $e->getMessage();
        error_log("Square refund error - encountered exception during refund for invoice ID $invoice_id / $invoice_prefix$invoice_number: $error");
        logApp("Square", "error", "Exception during refund for invoice ID $invoice_id: $error");
        flashAlert("Square refund failed: $error", 'error');
        redirect();
    }
    ...
    mysqli_query($mysqli,"INSERT INTO history SET history_status = '$invoice_status', history_description = 'Payment $square_payment_id deleted and refunded', history_invoice_id = $invoice_id");

    logAudit("Invoice", "Edit", "$session_name refunded and deleted Payment $square_payment_id on Invoice $invoice_prefix$invoice_number", $client_id, $invoice_id);

    flashAlert("Payment deleted and refunded in Square", 'error');
