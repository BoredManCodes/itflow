...
if (isset($_POST['add_payment_provider'])) {
    ...
    if ($provider === 'Square') {
        try {
            ...
            squareApiRequest('GET', '/v2/merchants', $private_key, squareIsSandbox($public_key));
        } catch (Exception $e) {
            flashAlert("Error checking Square access token: " . $e->getMessage(), 'error');
            redirect();
        }
    }
