...
if (isset($_POST['create_square_customer'])) {
    ...
    // Get Square provider config
    $square_provider_result = mysqli_query($mysqli, "
        SELECT payment_provider_id, payment_provider_private_key, payment_provider_public_key
        FROM payment_providers
    ...
    $square_access_token = escapeHtml($square_provider['payment_provider_private_key']);
    ...
    if (!$existing_customer) {
        try {
            ...
            error_log("Square error while creating customer for $client_name: $error");

            logApp("Square", "error", "Failed to create Square customer for $client_name: $error");
